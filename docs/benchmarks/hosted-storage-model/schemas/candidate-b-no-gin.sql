CREATE SCHEMA candidate_b_no_gin;

CREATE TABLE candidate_b_no_gin.collections (
...
CREATE TABLE candidate_b_no_gin.projection_generations (
  collection_id uuid NOT NULL REFERENCES candidate_b_no_gin.collections(collection_id) ON DELETE CASCADE,
...
ALTER TABLE candidate_b_no_gin.collections ADD CONSTRAINT collections_active_generation_fk
  FOREIGN KEY (collection_id, active_generation_id)
  REFERENCES candidate_b_no_gin.projection_generations(collection_id, generation_id)
  DEFERRABLE INITIALLY DEFERRED;

CREATE TABLE candidate_b_no_gin.records (
  collection_id uuid NOT NULL REFERENCES candidate_b_no_gin.collections(collection_id) ON DELETE CASCADE,
...
CREATE TABLE candidate_b_no_gin.record_projections (
  ...
  FOREIGN KEY (collection_id, record_id)
    REFERENCES candidate_b_no_gin.records(collection_id, record_id) ON DELETE CASCADE,
  FOREIGN KEY (collection_id, generation_id)
    REFERENCES candidate_b_no_gin.projection_generations(collection_id, generation_id)
);

CREATE TABLE candidate_b_no_gin.record_versions (
  collection_id uuid NOT NULL REFERENCES candidate_b_no_gin.collections(collection_id) ON DELETE CASCADE,
  record_id uuid NOT NULL,
  sequence bigint NOT NULL CHECK (sequence > 0),
  record_revision text NOT NULL,
...
CREATE TABLE candidate_b_no_gin.changes (
  collection_id uuid NOT NULL REFERENCES candidate_b_no_gin.collections(collection_id) ON DELETE CASCADE,
  sequence bigint NOT NULL CHECK (sequence > 0),
  record_id uuid NOT NULL,
...
-- records(collection_id, path_token). Projection JSON has no index.
