...
async function startObjects() {
  await execute("docker", ["run", "--rm", "-d", "--name", objects, "-e", `MINIO_ROOT_USER=${objectAccess}`, "-e", `MINIO_ROOT_PASSWORD=${objectSecret}`, "-p", "127.0.0.1::9000", (await buildS3TestImages()).minio, "server", "/data", "--address", ":9000"]);
...
async function mc(...args) { return (await execute("docker", ["run", "--rm", "--network", `container:${objects}`, "--entrypoint", "/bin/sh", (await buildS3TestImages()).mc, "-c", `mc alias set local http://127.0.0.1:9000 ${objectAccess} ${objectSecret} >/dev/null && mc ${args.join(" ")}`])).stdout; }
