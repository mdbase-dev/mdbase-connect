...
const beta98 = "52f6dad8dc3202461bd1ee244a0815cdef350c87";
const release95 = { id: 95, tag_name: "v0.1.0-beta.95", draft: false, published_at: "2026-09-08T00:00:00Z" };
const release98 = { ...release95, id: 98, tag_name: "v0.1.0-beta.98" };
...
`
  ...
  const tag = historical ? release95.tag_name : release98.tag_name;
  const commit = historical ? beta95 : beta98;
...
`], {
      ...
      env: {
        ...
        METADATA: typeof metadata === "string" ? metadata : JSON.stringify(metadata ?? (historical ? release95 : [release98, release95])),
...
test("ordinary predecessor verifies newest beta98 and its annotated origin tag", async (context) => {
  const result = await verify(context);
  assert.equal(result.code, 0, result.stderr);
  assert.match(result.calls, /releases\?per_page=100\ngit\n$/);
});

test("ordinary qualification cannot substitute the historical beta95 pin for newest beta98", async (context) => {
...
for (const [name, metadata] of [
  ["older pin", [release95, release98]],
  ["duplicate release", [release98, release98]],
  ["draft only", [{ ...release98, draft: true }]],
  ["missing release", []],
  ["object instead of inventory", release98],
...
for (const metadata of [
  { ...release95, draft: true }, { ...release95, tag_name: release98.tag_name },
