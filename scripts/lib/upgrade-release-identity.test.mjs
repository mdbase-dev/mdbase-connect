...
const beta100 = "6bd9420690295d3c5f65e68edcbf8e3e55612614";
const release95 = { id: 95, tag_name: "v0.1.0-beta.95", draft: false, published_at: "2026-09-08T00:00:00Z" };
const release100 = { ...release95, id: 100, tag_name: "v0.1.0-beta.100" };
...
`
  ...
  const tag = historical ? release95.tag_name : release100.tag_name;
  const commit = historical ? beta95 : beta100;
...
`], {
      ...
      env: {
        ...
        METADATA: typeof metadata === "string" ? metadata : JSON.stringify(metadata ?? (historical ? release95 : [release100, release95])),
...
test("ordinary predecessor verifies newest beta100 and its annotated origin tag", async (context) => {
  const result = await verify(context);
  assert.equal(result.code, 0, result.stderr);
  assert.match(result.calls, /releases\?per_page=100\ngit\n$/);
});

test("ordinary qualification cannot substitute the historical beta95 pin for newest beta100", async (context) => {
...
for (const [name, metadata] of [
  ["older pin", [release95, release100]],
  ["duplicate release", [release100, release100]],
  ["draft only", [{ ...release100, draft: true }]],
  ["missing release", []],
  ["object instead of inventory", release100],
...
for (const metadata of [
  { ...release95, draft: true }, { ...release95, tag_name: release100.tag_name },
