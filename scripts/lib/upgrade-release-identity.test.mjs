...
const beta99 = "c8b565f7dfbba6259e413b2c3bf2046325cde290";
const release95 = { id: 95, tag_name: "v0.1.0-beta.95", draft: false, published_at: "2026-09-08T00:00:00Z" };
const release99 = { ...release95, id: 99, tag_name: "v0.1.0-beta.99" };
...
`
  ...
  const tag = historical ? release95.tag_name : release99.tag_name;
  const commit = historical ? beta95 : beta99;
...
`], {
      ...
      env: {
        ...
        METADATA: typeof metadata === "string" ? metadata : JSON.stringify(metadata ?? (historical ? release95 : [release99, release95])),
...
test("ordinary predecessor verifies newest beta99 and its annotated origin tag", async (context) => {
  const result = await verify(context);
  assert.equal(result.code, 0, result.stderr);
  assert.match(result.calls, /releases\?per_page=100\ngit\n$/);
});

test("ordinary qualification cannot substitute the historical beta95 pin for newest beta99", async (context) => {
...
for (const [name, metadata] of [
  ["older pin", [release95, release99]],
  ["duplicate release", [release99, release99]],
  ["draft only", [{ ...release99, draft: true }]],
  ["missing release", []],
  ["object instead of inventory", release99],
...
for (const metadata of [
  { ...release95, draft: true }, { ...release95, tag_name: release99.tag_name },
