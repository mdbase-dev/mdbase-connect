...
export async function startConnectTestEnvironment(options = {}) {
  const environment = await createConnectEnvironment({
    ...
    serverImage: options.serverImage ?? "mdbase-connect-server:test",
    natsImage: options.natsImage ?? "mdbase-connect-nats:test"
  });
  await environment.up();
  return environment;
}

export async function createConnectEnvironment(options = {}) {
  ...
  const randomCredentials = options.randomizeCredentials === false
    ? {}
    : {
  ...
  const processEnvironment = {
    ...
    ...(options.serverImage
      ...
      : {}),
    ...(options.natsImage ? { MDBASE_CONNECT_NATS_IMAGE: options.natsImage } : {}),
  ...
  const manager = {
    ...
    async up() {
      ...
      try {
        ...
        await manager.diagnostics().catch(() => {});
        await manager.down({ volumes: options.disposable === true })
          .catch(() => {});
        throw error;
      }
      return manager;
    ...
    async logs() {
      return compose(["logs", "--follow", "--no-color"]);
    },
    async down({ volumes = false } = {}) {
      const arguments_ = ["down"];
      if (volumes) arguments_.push("--volumes");
      arguments_.push("--remove-orphans", "--timeout", "5");
      await compose(arguments_);
    },
    async reset() {
      await manager.down({ volumes: true });
      await manager.up();
    },
    async close() {
      if (closed) return;
