...
#[test]
fn trust_accept_requires_the_request_id_and_displayed_code() {
    let request_id = "01977777-7777-7777-8777-777777777777";
    let args = Args::try_parse_from([
        "mdbase",
        "connect",
        "trust",
    ...
    let (command, output) = control_command(command).unwrap();
    let ControlCommand::ApplicationTrustAccept(params) = command else {
        panic!("expected application-trust.accept")
    };
    assert_eq!(params.request_id.to_string(), request_id);
    assert_eq!(params.authentication_string, "SY8B-VQ53");
    assert!(matches!(output, OutputKind::TrustDetail));

    assert!(Args::try_parse_from(["mdbase", "connect", "trust", "accept", request_id,]).is_err());
}

#[test]
fn trust_output_contains_only_remote_identity_metadata() {
    let value = serde_json::json!({
        "pending": [{
            "request_id": "01977777-7777-7777-8777-777777777777",
            "presentation": { "application_name": "TaskNotes" },
            "authentication_string": "SY8B-VQ53"
        }],
        "trusted": []
    });
    let rendered = render_human(OutputKind::TrustList, &value);
    assert!(rendered.starts_with("APPLICATION"));
    assert!(rendered.contains("TaskNotes"));
    assert!(rendered.contains("SY8B-VQ53"));
