...
#[command(about = "Connect mdbase collections to applications and hosted storage")]
#[command(version)]
struct Args {
...
async fn execute(args: Args) -> Result<(), CliError> {
    ...
    match args.command {
        ...
        Command::Daemon(command) => {
            ...
            print_result(args.json, OutputKind::Daemon, &value)?;
            Ok(())
        }
        Command::Login {
            server,
            ...
            print_result(args.json, OutputKind::Account, &value)?;
            Ok(())
        }
        Command::Logout => {
        ...
        Command::Mirror(MirrorCommand::Promote {
            replica_id,
            no_open,
        }) => {
            ...
            print_result(args.json, OutputKind::Generic, &completed)?;
            Ok(())
        }
        Command::Doctor => {
            let value = doctor(&state_dir, &endpoint).await;
            print_result(args.json, OutputKind::Doctor, &value)?;
        ...
        command => {
            ...
            print_result(args.json, output, &result)
        }
    }
}
...
fn control_command(command: Command) -> Result<(ControlCommand, OutputKind), CliError> {
    let pair = match command {
        Command::Status => (ControlCommand::Status, OutputKind::Status),
        Command::Whoami => (ControlCommand::AccessSnapshot, OutputKind::Account),
        Command::Ping => (ControlCommand::Ping, OutputKind::Generic),
        Command::Collection(CollectionCommand::List) => {
        ...
        Command::Mirror(MirrorCommand::List) => (ControlCommand::MirrorList, OutputKind::Mirrors),
        Command::Mirror(MirrorCommand::Add {
            collection_id,
            path,
            name,
            read_only,
        ...
        Command::Mirror(MirrorCommand::Sync { replica_id }) => (
            ControlCommand::MirrorSync(MirrorIdParams { replica_id }),
            OutputKind::Mirror,
        ),
        Command::Mirror(MirrorCommand::Resolve {
            replica_id,
            record_id,
            r#use,
        }) => (
        ...
        Command::Mirror(MirrorCommand::Remove { replica_id, yes }) => {
            if !yes {
        ...
        Command::Hosted(HostedCommand::Delete { collection_id, yes }) => {
            if !yes {
                return Err(CliError::usage(
...
mod tests {
    ...
    fn invalid_operation_json_is_a_usage_error() {
        let result = control_command(Command::Operation {
