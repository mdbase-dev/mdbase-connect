...
fn transient_operation_status(status: StatusCode) -> bool {
    matches!(
        status,
...
fn unknown_hosted_mutation(operation: &str, request_id: Uuid) -> ConnectError {
    ConnectError::CloudProblem {
...
fn private_key(key: &SigningKey) -> String {
    URL_SAFE_NO_PAD.encode(key.to_bytes())
}

fn signing_key(value: &str) -> Result<SigningKey, ConnectError> {
...
fn validate_token_response(token: &TokenResponse) -> Result<(), ConnectError> {
...
fn api_value_error(value: Value, fallback: &str) -> ConnectError {
    let code = value
        .pointer("/error/code")
        .and_then(Value::as_str)
...
fn cloud_transport_error(error: reqwest::Error) -> ConnectError {
...
mod tests {
    ...
    async fn test_device_authorization(
        ...
        *state.grant_signing_public_key.lock().unwrap() =
            Some(proof.binding.grant_signing_public_key);
        Json(json!({
    ...
    fn token_validation_rejects_cross_collection_and_insecure_authority_urls() {
        ...
        let token = TokenResponse {
            ...
            authority: HostedAuthorityToken {
                operations_url: format!(
                    "https://sync.example/v1/authorities/{other_id}/operations"
                ),
