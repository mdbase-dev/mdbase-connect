...
impl HostedProvider {
    ...
    async fn projection_readiness_diagnostic(
        &self,
    ) -> ApiResult<ProjectionReadinessDiagnostic> {
        let row = sqlx::query(
            r#"SELECT
        ...
        .fetch_one(&self.pool)
        .await?;
        let count = |key: &str| -> ApiResult<u64> {
            number(row.get::<i64, _>(key), "diagnostic count")
        };
    ...
    async fn projection_progress_diagnostic(
        &self,
    ) -> ApiResult<Vec<ProjectionProgressDiagnostic>> {
    ...
    async fn storage_diagnostic(&self) -> ApiResult<StorageDiagnostic> {
        let (bucket, expiry) = self.blob_store.storage_diagnostic().ok_or_else(|| {
            ApiError::internal("The blob store reports no storage diagnostic.")
        })?;
        let expires_at = expiry.and_then(|value| DateTime::<Utc>::from_timestamp(value, 0));
        Ok(StorageDiagnostic {
            bucket,
            credential_expires_at: expires_at,
            credential_expires_in_seconds: expires_at
                .map(|at| (at - Utc::now()).num_seconds()),
        })
    }

    async fn recent_resource_changes_diagnostic(
        &self,
    ) -> ApiResult<Vec<ResourceChangeDiagnostic>> {
