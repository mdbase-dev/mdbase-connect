...
describe("ConnectApp", () => {
  ...
  it("keeps provider-pending revocations visible without claiming success", async () => {
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
      if (new URL(String(input)).pathname === "/v1/account/sessions") {
        return Response.json({ sessions: [] });
      }
      return Response.json({
        user: { id: "person", name: "Example Person", email: "person@example.com", login: null },
        ...
        connectors: [],
        collections: [{
          id: "collection", connector_id: "computer", local_id: "local",
          connector_name: "Home computer", display_name: "Garden notes",
          spec_version: "1", enabled: true, contracts: [],
